...
# MAGIC It should log a model for that in UC, where the model name comntains the station ID.
...
def train_model(df: pd.DataFrame) -> pd.DataFrame:
  ...
  df = df.sort_values(by="ts") # needed for time series splitting
  ...
  with mlflow.start_run(nested=True, run_name=f"Station {station_id}") as mlflow_run:
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, train_size=0.8,
      random_state=42, shuffle=False) # no shuffle for temporal data
    ...
    def get_latest_model_version(model_name_, client):
      """Helper function to get latest model version"""
      model_version_infos = client.search_model_versions(f"name = '{model_name_}'")
      return max([model_version_info.version for model_version_info in model_version_infos])
  
    client.set_registered_model_alias(
      registered_model_name,
      "Champion",
      get_latest_model_version(registered_model_name, client)
    )
...
metrics_df.write.saveAsTable(schema + ".training_data")
